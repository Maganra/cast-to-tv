...
# Args: <monitor> <port> [audio_input=default_output] [fps=30] [bitrate_kbps=6000]
...
BITRATE = sys.argv[5] if len(sys.argv) > 5 else "6000"

# CBR + constant frame rate make the stream perfectly steady, which lets the
# Chromecast play at its minimum buffer (~2s) instead of holding extra safety
# margin against bitrate/timestamp bursts (bursty QP/VFR streams measured
# 2-5s wandering latency; CBR+CFR pins it at ~2s).
CMD = (f'gpu-screen-recorder -w {MON} -f {FPS} -a {AUD} -k h264 -ac aac '
       f'-keyint 1 -bm cbr -q {BITRATE} -fm cfr -c matroska -o /dev/stdout 2>/dev/null '
